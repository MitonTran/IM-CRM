...
export function TargetForm({ teams, owners, period, start, end, defaultTeam }: {
  ...
  const subjects = [
    ...teams.map((item) => ({ ...item, value: `team:${item.id}`, label: `Nhóm · ${item.name}` })),
    ...owners.map((item) => ({ ...item, value: `user:${item.id}`, label: `Tư vấn viên · ${item.name}` })),
  ];
  ...
  return <form action={action} className="mt-5 grid gap-3 sm:grid-cols-2">
    ...
    <label className="text-xs font-bold text-[#617168]">Chỉ số<select name="metric" defaultValue="revenue_vnd" className="focus-ring mt-1 min-h-11 w-full rounded-xl border border-[#d8e2dc] bg-white px-3 text-sm"><option value="revenue_vnd">Doanh thu</option><option value="contact_attempts">Lần liên hệ</option><option value="successful_contacts">Liên hệ thành công</option><option value="appointments">Lịch hẹn</option><option value="consultations">Tư vấn</option><option value="won_customers">Khách đã chốt</option><option value="new_customers">Khách mới</option><option value="tasks_on_time">Công việc đúng hạn</option></select></label>
