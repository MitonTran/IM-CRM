...
export function ManagementForms({ customerId, owners }: { customerId: string; owners: OwnerOption[] }) {
  ...
  return <div className="space-y-5 border-t border-[#e4ebe7] pt-5"><form action={transferAction} className="space-y-3"><input type="hidden" name="customerId" value={customerId} /><h3 className="font-bold">Chuyển người phụ trách</h3><select className={control} name="ownerUserId" required defaultValue=""><option value="" disabled>Chọn tư vấn viên</option>{owners.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select className={control} name="taskPolicy" defaultValue="reassign"><option value="reassign">Chuyển các lịch chăm sóc đang mở</option><option value="cancel">Hủy các lịch chăm sóc đang mở</option></select><input className={control} name="reason" required minLength={3} placeholder="Lý do chuyển" /><Feedback state={transferState} /><Submit pending={transferPending}>{transferPending ? "Đang chuyển…" : "Chuyển khách"}</Submit></form>
    <form action={deleteAction} className="space-y-3 rounded-2xl border border-rose-100 bg-rose-50/50 p-4"><input type="hidden" name="customerId" value={customerId} /><h3 className="font-bold text-rose-800">Ngừng theo dõi khách hàng</h3><input className={control} name="reason" required minLength={3} placeholder="Lý do ngừng theo dõi" /><Feedback state={deleteState} /><Submit danger pending={deletePending}>{deletePending ? "Đang cập nhật…" : "Ngừng theo dõi"}</Submit></form></div>;
}
