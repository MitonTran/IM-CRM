...
const columns: DataColumn<CustomerListItem>[] = [
  { key: "name", label: "Khách hàng", render: (row) => <div><p className="font-bold text-[#20342b]">{row.fullName}</p><p className="mt-1 text-xs text-[#596961]">{row.phone ?? row.email ?? "—"}</p></div> },
  ...
  { key: "owner", label: "Phụ trách", render: (row) => <div><p className="font-semibold">{row.ownerName ?? "Chưa giao"}</p><p className="mt-1 text-xs text-[#596961]">{row.teamName}</p></div> },
