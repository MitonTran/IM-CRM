...
export function DocumentUploadForm({ role, teamId, teams, users, folders }: { role: "admin" | "leader"; teamId: string | null; teams: DocumentOption[]; users: DocumentOption[]; folders: DocumentOption[] }) {
  ...
  const scopes = role === "leader" && teamId ? [{ value: `team:${teamId}`, label: "Nhóm của tôi" }] : [
    { value: "organization:", label: "Toàn công ty" },
    ...teams.map((item) => ({ value: `team:${item.id}`, label: `Nhóm · ${item.name}` })),
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!(file instanceof File) || !file.size) { setOk(false); setMessage("Hãy chọn một tệp."); return; }
    ...
    setPending(true); setMessage("Đang kiểm tra tệp...");
    ...
    const checksum = await sha256(file); setMessage("Đang tải tài liệu...");
    ...
    if (error) { setPending(false); setOk(false); setMessage("Không thể tải tài liệu. Vui lòng thử lại."); return; }
...
export function FolderForm({ role, teamId, teams, users }: { role: "admin" | "leader"; teamId: string | null; teams: DocumentOption[]; users: DocumentOption[] }) {
  ...
  const options = role === "leader" && teamId ? [{ scope: "team", id: teamId, label: "Nhóm của tôi" }] : [{ scope: "organization", id: "", label: "Toàn công ty" }, ...teams.map((item) => ({ scope: "team", id: item.id, label: `Nhóm · ${item.name}` })), ...users.map((item) => ({ scope: "user", id: item.id, label: `Cá nhân · ${item.name}` }))];
...
export function DeleteDocumentForm({ documentId }: { documentId: string }) {
  ...
  return <form action={action} className="mt-4 border-t border-[#eadfd9] pt-4"><input type="hidden" name="documentId" value={documentId} /><label className="text-xs font-bold text-[#7d6158]">Lý do ngừng sử dụng<input name="reason" required minLength={3} maxLength={500} className="focus-ring mt-1 min-h-10 w-full rounded-xl border border-[#dfcfc8] px-3 text-sm" /></label><button disabled={pending} className="focus-ring mt-2 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#8b4f40] px-3 text-sm font-bold text-white"><Trash2 size={15} />{pending ? "Đang cập nhật..." : "Ngừng sử dụng"}</button>{state.message ? <p className="mt-2 text-xs font-semibold text-[#7d6158]">{state.message}</p> : null}</form>;
}
...
export function ExtractionActionForm({ versionId, status }: { versionId: string; status: "pending" | "failed" }) {
  ...
  return <form action={action} className="mt-3"><input type="hidden" name="versionId" value={versionId} /><button disabled={pending} className="focus-ring inline-flex min-h-9 items-center gap-2 rounded-lg bg-[#edf3e3] px-3 text-xs font-bold text-[#426043] disabled:opacity-50">{pending ? <LoaderCircle className="animate-spin" size={14} /> : <FileText size={14} />}{pending ? "Đang xử lý nội dung..." : status === "failed" ? "Xử lý lại nội dung" : "Xử lý nội dung"}</button>{state.message ? <p aria-live="polite" className={`mt-2 text-xs font-semibold ${state.ok ? "text-emerald-700" : "text-rose-700"}`}>{state.message}</p> : null}</form>;
}
