...
export function CustomerAnalysisPanel({ customerId, analyses, enabled, providerName }: { customerId: string; analyses: AiCustomerAnalysisItem[]; enabled: boolean; providerName: string }) {
  ...
  return <section className="rounded-2xl border border-[#dbe5df] bg-gradient-to-br from-[#f5f9f1] to-white p-4">
    <div className="flex items-start justify-between gap-4"><div className="flex gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-[#dfeecf] text-[#315c43]"><Bot size={19} /></span><div><h3 className="font-bold">Phân tích khách hàng bằng AI</h3><p className="mt-1 text-xs font-semibold text-[#6a796f]">{AI_DISCLAIMER}</p><p className="mt-1 text-[11px] text-[#7a8780]">Provider: {providerName}</p></div></div>{latest && <div className="rounded-2xl bg-white px-3 py-2 text-center shadow-sm"><p className="text-xl font-black text-[#176a4f]">{latest.lead_score}</p><p className="text-[10px] font-bold uppercase tracking-wide text-[#79867f]">Lead score</p></div>}</div>
    ...
    {latest ? <div className="mt-5 space-y-5 border-t border-[#dfe7e2] pt-5">
      ...
      {latest.evidence_activity_ids.length > 0 && <div><p className="text-xs font-bold text-[#66766d]">Dẫn chứng activity</p><div className="mt-2 flex flex-wrap gap-2">{latest.evidence_activity_ids.map((id, index) => <a key={id} href={`#activity-${id}`} className="focus-ring rounded-full border border-[#cfdbd4] bg-white px-2.5 py-1 text-xs font-bold text-[#176a4f] hover:bg-[#f1f6ef]">#{index + 1}</a>)}</div></div>}
