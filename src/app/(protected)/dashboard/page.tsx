  ...
  return <div className="mx-auto max-w-[1380px]">
    <header className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
      ...
      <form method="get" className="grid gap-2 rounded-2xl border border-[#dfe7e2] bg-white p-3 sm:grid-cols-3 xl:min-w-[630px]">
        <select name="period" defaultValue={query.period} className="focus-ring min-h-11 rounded-xl border border-[#d8e2dc] bg-white px-3 text-sm"><option value="day">Theo ngày</option><option value="month">Theo tháng</option><option value="year">Theo năm</option></select>
        <input type="date" name="anchor" defaultValue={query.anchor} className="focus-ring min-h-11 rounded-xl border border-[#d8e2dc] px-3 text-sm" />
        <button className="focus-ring min-h-11 rounded-xl bg-[#176a4f] px-4 text-sm font-bold text-white">Xem kỳ</button>
        {data.viewer.role !== "sale" ? <><select name="team" defaultValue={data.effectiveFilters.team} className="focus-ring min-h-11 rounded-xl border border-[#d8e2dc] bg-white px-3 text-sm" disabled={data.viewer.role === "leader"}><option value="">Mọi team</option>{data.teams.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select name="owner" defaultValue={data.effectiveFilters.user} className="focus-ring min-h-11 rounded-xl border border-[#d8e2dc] bg-white px-3 text-sm"><option value="">Mọi Sale</option>{data.owners.filter((item) => !data.effectiveFilters.team || item.teamId === data.effectiveFilters.team).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></> : null}
        <select name="source" defaultValue={data.effectiveFilters.source} className="focus-ring min-h-11 rounded-xl border border-[#d8e2dc] bg-white px-3 text-sm"><option value="">Mọi nguồn</option>{data.sources.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
      </form>
    </header>
    ...
    <section className="mt-5 grid gap-5 xl:grid-cols-[1.18fr_.82fr]">
      <div className="card-shadow rounded-[24px] border border-[#e0e7e2] bg-white p-6 sm:p-7">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#7a8981]">Phễu chuyển đổi</p><h2 className="mt-2 text-xl font-bold tracking-[-.02em]">Từ liên hệ đến đăng ký</h2></div><span className="grid size-11 place-items-center rounded-2xl bg-[#edf3e3] text-[#426043]"><ContactRound size={20} /></span></div>
    ...
    {data.viewer.role !== "sale" ? <section className="mt-5 grid gap-5 xl:grid-cols-[1.18fr_.82fr]">
      <div className="card-shadow overflow-hidden rounded-[24px] border border-[#e0e7e2] bg-white"><div className="flex items-center justify-between border-b border-[#e7ece9] p-6"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#7a8981]">Theo Sale</p><h2 className="mt-2 text-xl font-bold">Bảng hiệu suất</h2></div><Medal className="text-[#8b6b32]" size={22} /></div>
        {data.leaderboard.length ? <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-[#f7f9f7] text-xs uppercase tracking-[.08em] text-[#748179]"><tr><th className="px-6 py-3">Sale</th><th className="px-4 py-3">Liên hệ</th><th className="px-4 py-3">Thành công</th><th className="px-4 py-3">Khách thắng</th><th className="px-4 py-3">Quá hạn</th><th className="px-6 py-3 text-right">Doanh thu</th></tr></thead><tbody>{data.leaderboard.map((row, index) => <tr key={row.user_id} className="border-t border-[#edf1ee]"><td className="px-6 py-4 font-bold"><span className="mr-3 text-[#94a099]">{index + 1}</span>{row.full_name}</td><td className="px-4 py-4">{row.contact_attempts}</td><td className="px-4 py-4">{row.successful_contacts}</td><td className="px-4 py-4">{row.won_customers}</td><td className="px-4 py-4">{row.overdue_now}</td><td className="px-6 py-4 text-right font-bold">{money.format(row.revenue_vnd)}</td></tr>)}</tbody></table></div> : <p className="p-8 text-center text-sm text-[#7b8982]">Chưa có Sale trong phạm vi được chọn.</p>}
      </div>
      <div className="card-shadow rounded-[24px] border border-[#e0e7e2] bg-white p-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#7a8981]">Thiết lập</p><h2 className="mt-2 text-xl font-bold">Đặt mục tiêu KPI</h2><p className="mt-2 text-sm leading-6 text-[#718078]">Leader chỉ đặt cho team và Sale thuộc team của mình; Admin có thể đặt cho toàn hệ thống.</p><TargetForm teams={data.teams} owners={data.owners} period={query.period} start={bounds.startDate} end={bounds.endDate} defaultTeam={data.effectiveFilters.team} />{data.targets.length ? <div className="mt-5 border-t border-[#e7ece9] pt-4"><p className="text-xs font-bold uppercase tracking-[.1em] text-[#7a8981]">Mục tiêu đã đặt cho scope đang xem</p><div className="mt-3 flex flex-wrap gap-2">{data.targets.map((item) => <span key={item.metricCode} className="rounded-full bg-[#edf3e3] px-3 py-1.5 text-xs font-semibold text-[#426043]">{targetLabel(item.metricCode)}: {item.metricCode === "revenue_vnd" ? money.format(item.targetValue) : integer.format(item.targetValue)}</span>)}</div></div> : null}</div>
    </section> : null}
  </div>;
}
