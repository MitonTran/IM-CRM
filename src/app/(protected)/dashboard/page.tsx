...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="mx-auto max-w-[1380px]">
    <header className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
      <div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#728179]">Kết quả · {bounds.label}</p><h1 className="mt-2 text-3xl font-bold tracking-[-.035em] sm:text-4xl">Chào {firstName}, đây là kết quả bán hàng.</h1><p className="mt-2 text-sm text-[#718078]">Theo dõi kết quả từ hoạt động chăm sóc và giao dịch đã ghi nhận.</p></div>
      <form method="get" className="grid gap-2 rounded-2xl border border-[#dfe7e2] bg-white p-3 sm:grid-cols-3 xl:min-w-[630px]">
        ...
        {data.viewer.role !== "sale" ? <><select aria-label="Lọc theo nhóm" name="team" defaultValue={data.effectiveFilters.team} className="focus-ring min-h-11 rounded-xl border border-[#d8e2dc] bg-white px-3 text-sm" disabled={data.viewer.role === "leader"}><option value="">Mọi nhóm</option>{data.teams.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select aria-label="Lọc theo tư vấn viên" name="owner" defaultValue={data.effectiveFilters.user} className="focus-ring min-h-11 rounded-xl border border-[#d8e2dc] bg-white px-3 text-sm"><option value="">Mọi tư vấn viên</option>{data.owners.filter((item) => !data.effectiveFilters.team || item.teamId === data.effectiveFilters.team).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></> : null}
    ...
    <section className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Metric icon={ListChecks} label="Công việc quá hạn" value={integer.format(data.summary.overdue_now)} current={data.summary.overdue_now} previous={data.previous.overdue_now} href="/tasks" tone="rose" inverse />
    </section>
    ...
    <section className="mt-5 grid gap-5 xl:grid-cols-[1.18fr_.82fr]">
      <div className="card-shadow rounded-[24px] border border-[#e0e7e2] bg-white p-6 sm:p-7">
        ...
        <div className="mt-6 grid gap-3 border-t border-[#e7ece9] pt-5 sm:grid-cols-3"><Mini label="Lịch hẹn" value={data.summary.appointments} /><Mini label="Công việc đúng hạn" value={`${data.summary.tasks_on_time}/${data.summary.tasks_completed}`} /><Mini label="Nhập muộn" value={data.summary.late_entries} /></div>
      </div>
      ...
      <div className="relative overflow-hidden rounded-[24px] bg-[#173c2f] p-6 text-white sm:p-7">
        ...
        {revenueTarget ? <><p className="relative mt-2 text-2xl font-bold">{money.format(data.summary.revenue_vnd)}</p><p className="relative mt-1 text-sm text-emerald-50/65">trên {money.format(revenueTarget.targetValue)}</p><div className="relative mt-6 h-3 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#c7db91]" style={{ width: `${progress ?? 0}%` }} /></div><p className="relative mt-3 text-sm font-bold text-[#d9e8af]">{progress?.toFixed(1)}% hoàn thành</p></> : <><p className="relative mt-3 text-2xl font-bold">Chưa đặt mục tiêu</p><p className="relative mt-3 text-sm leading-6 text-emerald-50/65">Chọn một tư vấn viên hoặc nhóm để theo dõi tiến độ doanh thu của kỳ này.</p></>}
      </div>
    </section>

    {data.viewer.role !== "sale" ? <section className="mt-5 grid gap-5 xl:grid-cols-[1.18fr_.82fr]">
      <div className="card-shadow overflow-hidden rounded-[24px] border border-[#e0e7e2] bg-white"><div className="flex items-center justify-between border-b border-[#e7ece9] p-6"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#596961]">Theo tư vấn viên</p><h2 className="mt-2 text-xl font-bold">Bảng hiệu suất</h2></div><Medal className="text-[#8b6b32]" size={22} /></div>
        {data.leaderboard.length ? <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-[#f7f9f7] text-xs uppercase tracking-[.08em] text-[#596961]"><tr><th className="px-6 py-3">Tư vấn viên</th><th className="px-4 py-3">Liên hệ</th><th className="px-4 py-3">Thành công</th><th className="px-4 py-3">Khách đã chốt</th><th className="px-4 py-3">Quá hạn</th><th className="px-6 py-3 text-right">Doanh thu</th></tr></thead><tbody>{data.leaderboard.map((row, index) => <tr key={row.user_id} className="border-t border-[#edf1ee]"><td className="px-6 py-4 font-bold"><span className="mr-3 text-[#596961]">{index + 1}</span>{row.full_name}</td><td className="px-4 py-4">{row.contact_attempts}</td><td className="px-4 py-4">{row.successful_contacts}</td><td className="px-4 py-4">{row.won_customers}</td><td className="px-4 py-4">{row.overdue_now}</td><td className="px-6 py-4 text-right font-bold">{money.format(row.revenue_vnd)}</td></tr>)}</tbody></table></div> : <p className="p-8 text-center text-sm text-[#596961]">Chưa có tư vấn viên trong phạm vi được chọn.</p>}
      </div>
      <div className="card-shadow rounded-[24px] border border-[#e0e7e2] bg-white p-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#596961]">Mục tiêu</p><h2 className="mt-2 text-xl font-bold">Giao mục tiêu</h2><p className="mt-2 text-sm leading-6 text-[#596961]">Chọn nhóm hoặc tư vấn viên và nhập kết quả cần đạt.</p><TargetForm teams={data.teams} owners={data.owners} period={query.period} start={bounds.startDate} end={bounds.endDate} defaultTeam={data.effectiveFilters.team} />{data.targets.length ? <div className="mt-5 border-t border-[#e7ece9] pt-4"><p className="text-xs font-bold uppercase tracking-[.1em] text-[#596961]">Mục tiêu trong kỳ đang xem</p><div className="mt-3 flex flex-wrap gap-2">{data.targets.map((item) => <span key={item.metricCode} className="rounded-full bg-[#edf3e3] px-3 py-1.5 text-xs font-semibold text-[#426043]">{targetLabel(item.metricCode)}: {item.metricCode === "revenue_vnd" ? money.format(item.targetValue) : integer.format(item.targetValue)}</span>)}</div></div> : null}</div>
    </section> : null}
  </div>;
}
...
function targetLabel(metric: string) { return ({ new_customers: "Khách mới", contact_attempts: "Liên hệ", successful_contacts: "Liên hệ thành công", appointments: "Lịch hẹn", consultations: "Tư vấn", tasks_on_time: "Công việc đúng hạn", won_customers: "Khách đã chốt", revenue_vnd: "Doanh thu" } as Record<string, string>)[metric] ?? metric; }
