  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    <section className="card-shadow mt-7 overflow-hidden rounded-[22px] border border-[#dfe7e2] bg-white">
      <form method="get" className="grid gap-3 border-b border-[#e4ebe7] p-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label><span className="sr-only">Team</span><select name="team" defaultValue={query.team} className="focus-ring min-h-11 w-full rounded-xl border border-[#d9e2dd] bg-white px-3 text-sm"><option value="">Mọi team</option>{pageData.teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label>
      ...
      <footer className="flex items-center justify-between px-4 py-4 text-sm text-[#69776f]"><span>Trang {Math.min(query.page, totalPages)} / {totalPages}</span><div className="flex gap-2"><Link aria-disabled={query.page <= 1} tabIndex={query.page <= 1 ? -1 : 0} href={`/customers?${customerQueryString(query, { page: Math.max(1, query.page - 1), customer: null })}`} className={`focus-ring grid size-10 place-items-center rounded-xl border border-[#dbe3de] ${query.page <= 1 ? "pointer-events-none opacity-40" : "hover:bg-[#f4f8f4]"}`}><ChevronLeft size={17} /></Link><Link aria-disabled={query.page >= totalPages} tabIndex={query.page >= totalPages ? -1 : 0} href={`/customers?${customerQueryString(query, { page: Math.min(totalPages, query.page + 1), customer: null })}`} className={`focus-ring grid size-10 place-items-center rounded-xl border border-[#dbe3de] ${query.page >= totalPages ? "pointer-events-none opacity-40" : "hover:bg-[#f4f8f4]"}`}><ChevronRight size={17} /></Link></div></footer>
    </section>
    ...
    {query.customer && <Drawer title={detail?.fullName ?? "Không tìm thấy khách hàng"} closeHref={closeDetailHref}>{detail ? <div className="space-y-7"><div className="flex items-center gap-3 rounded-2xl bg-[#f4f8f3] p-4"><span className="grid size-11 place-items-center rounded-2xl bg-[#deedca] text-[#315b43]"><UsersRound size={20} /></span><div><p className="text-sm font-bold">{detail.ownerName ?? "Chưa giao"}</p><p className="mt-0.5 text-xs text-[#738179]">{detail.teamName} · {detail.sourceName}</p></div></div><CustomerAnalysisPanel customerId={detail.id} analyses={detail.aiAnalyses} enabled={detail.aiEnabled} providerName={detail.aiProviderName} /><CustomerJourney customerId={detail.id} activities={detail.activities} followUps={detail.followUps} /><CustomerDeals customerId={detail.id} deals={detail.deals} role={pageData.viewer.role} idempotencyKey={crypto.randomUUID()} /><details className="rounded-2xl border border-[#dfe7e2] bg-white p-4"><summary className="focus-ring cursor-pointer list-none rounded-lg font-bold">Thông tin khách hàng</summary><div className="mt-5"><EditCustomerForm customer={detail} sources={pageData.sources} tags={pageData.tags} /></div></details>{pageData.viewer.role !== "sale" && <ManagementForms customerId={detail.id} owners={pageData.owners} />}</div> : <p className="rounded-2xl bg-amber-50 p-4 text-sm text-amber-800">Bản ghi không tồn tại hoặc bạn không có quyền xem.</p>}</Drawer>}
  </div>;
}

function Drawer({ title, closeHref, children }: { title: string; closeHref: string; children: React.ReactNode }) {
  return <div className="fixed inset-0 z-50 flex justify-end bg-[#10281e]/30 backdrop-blur-[2px]"><Link href={closeHref} aria-label="Đóng" className="absolute inset-0" /><aside role="dialog" aria-modal="true" aria-label={title} className="relative h-full w-full overflow-y-auto border-l border-[#dce5df] bg-[#fbfcfb] p-5 shadow-2xl sm:max-w-xl sm:p-7"><header className="mb-6 flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.13em] text-[#78867e]">IM CRM</p><h2 className="mt-2 text-2xl font-bold tracking-[-.025em]">{title}</h2></div><Link href={closeHref} aria-label="Đóng" className="focus-ring grid size-10 shrink-0 place-items-center rounded-xl border border-[#dbe4de] bg-white hover:bg-[#f2f6f3]"><X size={18} /></Link></header>{children}</aside></div>;
}
