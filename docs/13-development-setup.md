...
Workflow `Hosted restore drill` cần thêm secrets `RESTORE_DRILL_DB_URL`, `RESTORE_DRILL_SUPABASE_URL`, `RESTORE_DRILL_SERVICE_ROLE_KEY` và variables `RESTORE_DRILL_PROJECT_REF`, `RESTORE_DRILL_BACKUP_RUN_ID`. Người chạy phải nhập lại chính xác target ref và chọn `operation=restore` cho lần khôi phục đầu tiên hoặc `operation=uat` để kiểm tra project đã khôi phục. Chế độ restore dừng trước khi ghi nếu target trùng nguồn, URL/ref không khớp hoặc project đích đã có bảng `public`, Auth user, bucket hay Storage object. Database được restore theo quy trình `psql` một transaction của Supabase; Storage chỉ được upload qua API với `upsert` rồi tải lại để đối chiếu kích thước/SHA-256.

Chế độ `uat` không restore lại database/Storage. Nó bắt buộc đúng năm hồ sơ hoạt động gồm hai tư vấn viên, hai trưởng nhóm và một Admin trên hai nhóm; tạo mật khẩu ngẫu nhiên tạm thời chỉ trên project disposable, đăng nhập từng tài khoản qua Supabase Auth rồi đối chiếu chính xác RLS nhân sự, nhóm, khách hàng và tài liệu. Report chỉ chứa số đếm/vai trò đã ẩn danh, không chứa email, UUID, mật khẩu hay nội dung bản ghi. Project đích phải được dọn sau khi bằng chứng đã được lưu; không chạy chế độ này trên Preview nguồn hoặc Production.
