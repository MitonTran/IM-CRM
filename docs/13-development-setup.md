...
Workflow cũng chạy `npm run audit:preview-performance` bằng Chromium cho `/login`, `/dashboard` và `/customers`, mỗi route ba lượt context lạnh rồi lấy median của TTFB, FCP, LCP, CLS và transfer size. Tạo một tài khoản Admin UAT chỉ chứa dữ liệu giả trên Supabase Preview, lưu credential vào GitHub Actions secrets `PREVIEW_AUDIT_EMAIL` và `PREVIEW_AUDIT_PASSWORD`; report không chứa email, mật khẩu hoặc bypass secret. Artifact `preview-verification` giữ `deployment-smoke.json` và `preview-performance.json` trong 14 ngày. Đây là lab guardrail; trước Production vẫn phải lưu Lighthouse hoặc Speed Insights field data theo `docs/16-uat-checklist.md`.
